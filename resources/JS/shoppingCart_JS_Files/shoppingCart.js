...
Object.keys(updateProductPrices_SC).forEach((product) => {
    ...
    if (productPriceSuperDeals) {
        document.getElementById(priceElementId).textContent = productPriceSuperDeals;
...
document.querySelector(".cart-header-delete-items-text").addEventListener("click", function () {
    document.getElementById("custom-prompt-co").style.display = "block";
});

document.querySelector("#cancel-btn-prompt").addEventListener("click", function () {
...
document.querySelector("#close-btn-x-symbol").addEventListener("click", function () {
    document.getElementById("custom-prompt-co").style.display = "none";
});

document.getElementById('confirm-btn-prompt').addEventListener('click', function () {
    document.getElementById('custom-prompt-co').style.display = 'none';
    location.reload();
});
...
const productNames = ['one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine', 'ten', 'eleven'];
...
document.getElementById('confirm-btn-prompt').addEventListener('click', function () {
    ...
    productNames.forEach((productName, index) => {
        ...
        if (document.querySelector(productClass).style.zIndex === '' ||
            ...
            localStorage.removeItem(`count${productName.charAt(0).toUpperCase() + productName.slice(1)}`);
            localStorage.removeItem(`imgData${productName.charAt(0).toUpperCase() + productName.slice(1)}`);
            localStorage.removeItem(`imgData${productName.charAt(0).toUpperCase() + productName.slice(1)}_TEMP`);
            const keys = Object.keys(localStorage).filter(key => key.startsWith(`productPrice${productName.charAt(0).toUpperCase() + productName.slice(1)}`));
            keys.forEach(key => localStorage.removeItem(key));
            localStorage.removeItem(`productTitle${productName.charAt(0).toUpperCase() + productName.slice(1)}`);
